...
class UserModel extends Model {
     ...
     * 查询服务员
     */
    public function getWiteruser() {
        $where['role_id'] = 6;
        $where['c_id'] = $_SESSION['think']['c_id'];
        return $this->where($where)->order('id desc')->column('id,user_name');
    }

    /**
     * 查询服务员名称
     */
    public function getnameWiteruser($id) {
        $where['id'] = $id;
        $where['role_id'] = 6;
    ...
    public function getadminuser() {
        $where['role_id'] = 3;
        return $this->where($where)->column('id,user_name');
    }
